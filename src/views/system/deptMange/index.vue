...
import { deptList, deptDel } from '@/utils/request/api'
...
const dataAll = reactive({
  configsOfTable: { //表格--
    ...
    tableConfig: {
      ...
      columData: [
        { prop: 'name', label: '部门名', search: true, type: 'input', form_hide: false, placeholder: '****', show: 'text' },
        { prop: 'status', label: '部门状态', align: 'center', show: 'tagNum' },
        { prop: 'linkMan', label: '联系人', search: false, type: 'input', placeholder: '****', show: 'text' },
        { prop: 'linkPhone', label: '联系电话', search: false, type: 'input', placeholder: '****', show: 'text' },
      ],
      operationData: [{ operationName: '修改', btnType: '', authstr: 'update-dept' },
      { operationName: '删除', btnType: 'danger', authstr: 'delete-dept' },],
      treeprops: { children: 'children', hasChildren: 'hasChildren' },


    },
    toolsConfig: {
      addBtn: { isShow: true, authstr: 'save-dept' },//新增
      delBtn: { isShow: true, authstr: 'delete-dept' },//删除（多个）
  ...
  tableEvent: {
    ...
    dels(ctx) {
      ...
      ElMessageBox.confirm(`确定删除菜单'${delNames}'吗?`, '删除提示',
        {
    ...
    query(ctx) {
      ...
      deptList(ctx).then(res => {
        configsOfTable.value.tableConfig.loading = false
        configsOfTable.value.data = tableEvent.value.toTree(res.data.records)
        if (ctx.size != -1) {
