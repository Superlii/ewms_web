<template>
    ...
    <div style="width: 100%;">
        ...
        <div ref="form">
            <a-form class="form" :model="formInline">
                <a-row :gutter="[16,0]" justify="start">
                    <a-col v-for="(item, index) in searchForm " :key="index" v-show="item.form_hide ? open_ : true"
                        :span="6">
                        <a-form-item :label="item.label + ':'">

                            <!-- 默认表单 -->

                            <a-input @keyup.enter="onSubmit" size="small" v-if="item.type == 'input'"
                                v-model:value="formInline[item.prop]" :placeholder="item.placeholder">
                            </a-input>
                    ...
                    <a-col :span="1.5">
                        <a-form-item>
                            <a-button v-oneOpt="1000" size="small" type="primary" @click="onSubmit"><template #icon>
                                    <allIcon.SearchOutlined />
                                </template>查询
                            </a-button>
                        </a-form-item>
                    </a-col>
                    <a-col :span="1.5">
                        <a-form-item>
                            <a-button v-oneOpt="1000" size="small" @click="onRest"><template #icon>
                                    <allIcon.ReloadOutlined />
                                </template>重置
                            </a-button>
                        </a-form-item>
                    </a-col>
                    <a-col v-show="seacrhDataFilter(tableConfig.columData).length > 4" :span="1.5">
                        <a-form-item>
                            <a-button style="font-size: 12px;color: #333;" size="small" type="text"
                                @click="hideformitem">
                                展开/折叠
                                <allIcon.CaretDownOutlined :style="{color:'#409eff'}" v-show="!open_" />
                                <allIcon.CaretUpOutlined :style="{color:'#409eff'}" v-show="open_" />
                            </a-button>
                        </a-form-item>
                    </a-col>
                </a-row>
            ...
            <div class="toolitem">
                <a-row :gutter="[16,0]">
                    <a-col class="gutter-row" :span="12">
                        <a-button v-oneOpt="1000" v-hasBtn="toolsConfig.addBtn.authstr" v-if="toolsConfig.addBtn.isShow"
                            @click="adds" size="small" type="primary"><template #icon>
                                <allIcon.PlusOutlined />
                            </template>新增
                        </a-button>
                    </a-col>
                    <a-col class="gutter-row" :span="12">
                        <a-button v-oneOpt="1000" v-hasBtn="toolsConfig.addBtn.authstr" v-if="toolsConfig.delBtn"
                            @click="delets" size="small" danger><template #icon>
                                <allIcon.DeleteOutlined />
                            </template>删除
                        </a-button>
                    </a-col>
                </a-row>
            ...
            <div class="toolitem">
                <a-row :gutter="16">
                    <a-col class="gutter-row" :span="4">
                        <a-tooltip v-if="toolsConfig.uploads.isShow" :title="toolsConfig.uploads.tooltipCon"
                            placement="top">
                    ...
                    <a-col class="gutter-row" :span="4">
                        <a-tooltip v-if="toolsConfig.downloads.isShow" :title="toolsConfig.downloads.tooltipCon"
                            placement="top">
                            <a-button @click="downloadFile(toolsConfig.downloads.name)" shape="circle" size="small">
                                <template #icon>
                    ...
                    <a-col class="gutter-row" :span="4">
                        <a-tooltip v-if="toolsConfig.printTableData.isShow"
                            :title="toolsConfig.printTableData.tooltipCon" placement="top">
                    ...
                    <a-col class="gutter-row" :span="4">
                        <a-tooltip v-if="toolsConfig.refreshTable.isShow" :title="toolsConfig.refreshTable.tooltipCon"
                            placement="top">
                    ...
                    <a-col class="gutter-row" :span="4">
                        <a-tooltip v-if="toolsConfig.fullScreen.isShow" :title="toolsConfig.fullScreen.tooltipCon"
                            placement="top">
                    ...
                    <a-col class="gutter-row" :span="4">
                        <a-tooltip v-if="toolsConfig.columnsFilter.isShow" auto-close
                            :content="toolsConfig.columnsFilter.tooltipCon" effect="customized" placement="top">
                ...
                <div v-show="columnBoxbtn" class="columnBox">
                    <div class="head">
                        <a-checkbox :checked="checkAll" :indeterminate="isIndeterminate" @change="handleCheckAllChange">
                            全选
                        </a-checkbox>
                    </div>
                    <div class="main">
                        <a-checkbox-group :value="checkedNow" :options="plainOptions"
        ...
        <a-table :scroll="{ x: 1500}" :row-selection="rowSelection" :row-key="(record)=>record.id"
            v-loading="tableConfig.loading" :pagination="false" class="table" :dataSource="data"
            :columns="dataFilter(tableConfig.columData)">
            <template #headerCell="{ column }">
                <template v-if="column.key === 'name'">
                    <span>
                        <allIcon.SmileOutlined />
                        菜单名称
                    </span>
                </template>
            </template>
            ...
            <template #bodyCell="{ column, record }">
                ...
                <template v-else-if="column.key === 'icon'">
                    <span>
                        <svg-icon v-if="!!record.icon" size="2rem" :name="record.icon || ''"></svg-icon>
                        <span v-else>{{ '--' }}</span>
                    </span>
                </template>
                <template v-else-if="column.key == '更多...'">
                    <a-tooltip :title="'查看'" placement="top">
                        <a-button @click="tableBtnClick(record,column.key)" type="text" size="small">
                            查看({{record.userName}})
                        </a-button>
                    </a-tooltip>
                </template>
                <template v-else-if="column.key === 'operation'">
                    <a-row :gutter="[8,8]">
                        <a-col :span="8" v-for="(item, index) in tableConfig.operationData.filter(i=>!i.isHide)"
                            :key="index">
                            <a-button v-oneOpt="1000" v-hasBtn="item.authstr" style="margin-left: 5px;"
                                v-if="!item.isHide" :type="item.btnType || ''" size="small"
                                @click="operatiosFun(record, $index, item.operationName)">{{
                                item.operationName
                                }}</a-button>

                        </a-col>
                        <a-col :span="8" v-if="tableConfig.operationData.length > 2">
                            <!-- 按钮较多处理(大于2个放下拉菜单) -->

                            <a-dropdown class="drops">
                                <span class="el-dropdown-link con">
                                    <el-button type="default" size="small">更多
                                        <allIcon.CaretDownOutlined :style="{color:'#409eff'}" />
                                    </el-button>
                                ...
                                <template #overlay>
                                    <a-menu>
                                        <template v-for="(item, index) in tableConfig.operationData" :key="index">
                                            <a-menu-item v-if="item.isHide">

                                                <a-button v-oneOpt="1000" v-hasBtn="item.authstr"
                                                    :icon="allIcon[item.icon] || ''" :type="item.btnType || ''"
                                                    size="small"
                                                    @click="operatiosFun(record, $index, item.operationName)">
                                                    {{
                                                    item.operationName
        ...
        <!-- 分页 -->
        <a-pagination v-show="pagination.isShow" size="small" :total="pagination.total" show-size-changer
            v-model:current="pagination.current" v-model:pageSize="pagination.size"
            :pageSizeOptions="pagination.pageSizeOptions" show-quick-jumper :show-total="pagination.showTotal"
...
checkedNow.value = allData.map(i => { return i.label })
plainOptions.value = allData.map(i => { return i.label })
