<template>
  ...
  <div>
    <a-modal
      ...
      :title="dialogtype == 'add' ? '新增库区' : '修改库区'"
    >
      <a-form style="padding-left: 200px" :model="form" :rules="rules" ref="ruleFormRef">
        <a-row :gutter="20">
          <a-col>
            <a-form-item
...
import { reservoirAdd, reservoirEdit, reservoirDetail } from "@/utils/request/api";
...
const rules = reactive({
  name: [{ required: true, message: "请设置库区名", trigger: "blur" }],
  leader: [{ required: true, message: "请设置库区负责人", trigger: "blur" }],
  organization: [
    { required: true, message: "请设置库区所属组织", trigger: "blur" },
  ],
});
...
watch(dialogVisible, (n, o) => {
  if (n) {
    ...
    if (dialogtype.value == "add") {
      ...
      reservoirDetail(objdata.id).then((res) => {
